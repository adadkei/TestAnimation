...
public class State extends JLabel implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (dc.OrNotSubPreVertex() && CheckMethodAndFieldFinder.is_on_clicked && cm.checkOnFlow()
                && cm.subCheckHeatingFlow()
                || (CheckMethodAndFieldFinder.is_on_clicked && cm.checkOnToBoiling())) {
        ...
        // 保温中表示
        if (dc.OrNotSubPreVertex() && cm.subCheckBoilingToKeepWarmFlow()
                || (cm.checkBoilingToKeepWarmFlow())) {
    ...
    public boolean isTriggerON() {
        if (dc.checkPreVertex("off", "開始擬似状態0") && dc.checkPreVertex("on", "off")
                && dc.checkVertexIncoming("on", kara)) {
            return true;
        }
        return false;
    }
