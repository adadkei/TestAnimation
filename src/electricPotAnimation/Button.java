...
public class Button implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource().equals(button)) {// ボタンが押された時のアクション
			...
			} else {// on→加熱中の流れがある場合、または保温中→off
				if (button.getText().equals(k1)&&(cm.generalPurposeCheckVertexAndTrigger("off", "加熱中", "OFF")
						|| cm.generalPurposeCheckVertexAndTrigger("off", "保温中", "OFF"))) {
					allStateToOff();
				}
			}
