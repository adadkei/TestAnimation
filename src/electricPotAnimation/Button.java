...
public class Button implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource().equals(button)) {// ボタンが押された時のアクション
             ...
             * OFFが押された時のながれ
             */
            if (!dc.checkPreVertex("加熱中", "on")) {// もし、on→加熱中の流れがなかった場合
                if ((cm.checkOffFlow()) && button.getText().equals(k1)) {
                    allStateToOff();
                }
            } else {// on→加熱中の流れがあり、かつ加熱中だったら
                if (button.getText().equals(k1)
                        && (dc.generalPurposeCheckVertexAndTrigger("off", "加熱中", "OFF") && CheckMethodAndFieldFinder.is_boiling)) {
                    allStateToOff();// すべての状態をfalseにする
                }
                if (button.getText().equals(k1)
                        && dc.generalPurposeCheckVertexAndTrigger("off", "保温中", "OFF")
                        && CheckMethodAndFieldFinder.is_keep_warm) {// 保温中→offがあり、保温中だったら
                    allStateToOff();
                }
            }
             ...
             * 沸騰ボタンが押された時の流れ
             */
            if ((cm.checkKeepWarmToBoilingFlow() || cm.subCheckKeepWarmToBoilingFlow())
                    && button.getText().equals(k3) && !CheckMethodAndFieldFinder.is_boiling) {
