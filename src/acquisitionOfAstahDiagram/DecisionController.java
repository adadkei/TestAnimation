...
public class DecisionController {
    ...
    public boolean checkPreVertex(String current_vertex, String check_pre_vertex) {
        ...
        for (int i = 0; i < ivx.length; i++) {// 全ての状態でループを回す
            ...
            if (ivx[i].toString().startsWith(check_pre_vertex)) {// 指定した状態と名前が一致したら。スペースがあってもこれで通る
                return true;
            }
    ...
    public boolean checkVertexIncoming(String state_name, String checking_tri) {
        ...
        for (int i = 0; i < it.length; i++) {
            if (it[i].toString().startsWith(checking_tri)) {
                return true;
            }
        }
        return false;
     ...
     */
    public boolean checkSubVertexIncoming(String parent_v_name, String sub_v_name,
            String checking_tri) {
        ITransition[] it = new ITransition[4];// 仮配列
     ...
     * @return boolean
     */
    public boolean checkPreSubVertex(String parent_state_name, String sub_vertex,
            String sub_pre_vertex) {
        ...
        for (int i = 0; i < pre_subVertexes.length; i++) {
            if (pre_subVertexes[i].toString().startsWith(sub_pre_vertex)) {
                return true;
            }
            // 開始状態判別用。"開始"で始めればOKにするため。
     ...
     * @return boolean
     */
    public boolean generalPurposeCheckVertexAndTrigger(String current_vertex, String pre_vertex,
            String trigger) {
        if (checkPreVertex(current_vertex, pre_vertex)
                && checkVertexIncoming(current_vertex, trigger)) {
            return true;
        }
        return false;
    }
